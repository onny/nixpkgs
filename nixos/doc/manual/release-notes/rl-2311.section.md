...
- Following options of the Nextcloud module are now covered by `extraOptions` and should be migrated: `logLevel` (now `extraOptions.loglevel`), `logType` (`extraOptions.log_type`), `defaultPhoneRegion` (`extraOptions.default_phone_region`), `overwriteProtocol` (`extraOptions.overwriteProtocol`), `skeletonDirectory` (`extraOptions.skeletondirectory`), `globalProfiles` (`extraOptions.profile.enabled`), `extraTrustedDomains` (`extraOptions.trusted_domains`) and `trustedProxies` (`èxtraOptions.trusted_proxies`), `objectstore` (now `extraOptions.objectstore`).
