...
python3Packages.buildPythonApplication {
  ...
  postPatch = ''
    ...
    # Fix hardcoded paths and compatibility issues
    substituteInPlace src/rspamd_trainer/run.py \
      --replace "from settings" "from .settings" \
      --replace "/usr/bin/rspamc" "${rspamd}/bin/rspamc"
  '';
